...
while (True):
    ...
    stereo = cv2.StereoBM_create(numDisparities=32, blockSize=blocksize)
    disparity = stereo.compute(grayRectL,grayRectR).astype(np.float32)
    disparity = cv2.normalize(disparity, disparity, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_8U)
    #disparity_denoise = cv2.fastNlMeansDenoising(disparity,templateWindowSize=7, searchWindowSize=21)

    ret,thres = cv2.threshold(disparity,50,255,0)
    disparity_cont, contours, hierarchy, = cv2.findContours(thres,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
    ...
    cv2.imshow('contours', disparity_cont)
